...
def dispatch_pull_request(ctx: PullRequestContext) -> dict[str, str]:
    ...
    try:
        ...
        # The fetcher is the only impure boundary of the evaluation:
        # evaluate_pull_request itself stays pure (spec 0002) -- the
        # fetcher is a parameter, not a call inside the pure function.
        # Built by the SAME builder the `/grug recheck` path uses (#782),
        # so the two paths cannot drift back into different verdicts.
