...
| **Persona** | A bounded behavior surface Grug applies per PR — a single grug of the tribe, each with one job. Today: **Chief** (Hunt Plan gate), **Elder** (code review), **Guard** (security), **Teller** (walkthrough), **Warder** (release), **Pulse** (stale nudge), **Smasher** (mutation trial). Personas are per-repo togglable. Each has a **caveman name** used on product surfaces and a **historical code key** (`tpm`, `code_reviewer`, …) in dirs/config. New code uses the caveman name; the key→name map is one write-boundary map. _Avoid_: code keys on user-facing surfaces. |
...
| **Chief** (the **TPM** persona) | The grug who leads the tribe. Before the tribe leave the cave to hunt, Chief ask: the hunt have a name? enough meat counted? the path home known? If the plan not whole, tribe not go. Chief not read the code — Chief check the **Hunt Plan**. = static body checks (`Grug - Chief`; legacy alias `Grug - Definition of Ready`), which block merge on a malformed PR body. Code dir `personas/tpm/` (key `tpm`); canonical name **`chief`**. The companion **Scope review** half is wired but NOT shipped — see "Roadmap" rows. See [`services/_shared/personas/tpm/`](services/_shared/personas/tpm/). |
| **Markings Board** | Elder's structured findings surface: severity, effort, What Elder sees, Fix, Lore, agent prompt. Check-run summary table + inline comments share the same shape. |
| **Swift Hunt** | Adaptive quiet window for Elder: tiny PRs settle 0s, medium cap at 5s, large keep the full base settle. Size comes from GitHub `changed_files` + additions/deletions; missing stats never invent a tiny path. |
| **Lore** | Prior tribe review history (ledger) used as measured confidence, precedent citations, few-shot examples, and auto-derived team practices. |
| **Elder** (the **code-reviewer** persona) | The oldest grug, many winters of bad code behind him — Elder seen many a grug fall in the same pit. Elder read your markings one line at a time and name the bad omen before it bite: the null in the dark, the race by the river, the error swallowed in silence. = the LLM **code review** pass (`Grug - Elder`; legacy alias `Grug - Code Review`). Apex path: **tiered Cave review** (ADR-0019 / #645) plus bounded staging (ADR-0021), Swift Hunt settle, in_progress check on enqueue, Markings Board publish, Lore + Omen fused. Production review jobs enter the durable rerun FIFO, wait for a 90-second quiet snapshot, and cancel/re-enqueue when base, head, title, or body changes before publication. Default `GRUG_REVIEW_DEPTH=tiered` runs bounded Cave **coder** discovery on the cold Spark; the Cave **reasoner** discovery arm joins only on escalation (large diff, high-risk paths, explicit deep-review, or calibration sample). Merged candidates are adjudicated again as file-scoped evidence packets by the permanently hot reasoner on sparkicus. `deep` restores always-on dual-arm discovery; `fast` is coder-first with reasoner only if coder fails. SaaS (OpenRouter review-only Opus / Poolside) is overload fallback; the judge's fallback is secret-redacted. Write-authorized maintainer reaction labels fan out only to the producer spans that actually exist. Confirmed reactions refresh positive practices/examples; false positives become bounded AVOID guidance. Teller and `/grug ask` keep the lighter OpenRouter model. (The deterministic security suite that used to ride inside Elder is now **Guard**, #466/ADR-0012.) Code dir `personas/code_reviewer/` (key `code_reviewer`); canonical name **`elder`** (already `_PERSONA = "Elder"` in dispatch). Speaks in the wise-caveman voice (the `_VOICE` clause in `code_review_prompt.py`). |
...
| **CheckResult** | Outcome of one plan check against one PR body. Frozen dataclass — fields `name: str`, `passed: bool`, `detail: str`, `skipped: bool` (default `False`). Pass/fail only — no third "warn" state. `skipped=True` marks a pass that was NOT earned: the check fell open because it could not be evaluated (`linked-issue-completeness` with no fetcher or a failed issue fetch, #782). Skipped implies passed (enforced in `__post_init__`); it never blocks, but the Chief title renders it as `skipped` and drops it from the "all N checks" claim. Whether a failed check blocks merge is decided at rollup time by `_ADVISORY_CHECKS`, not by the check itself. |
| **TpmEvaluation** | Aggregate result of running all plan checks against one PR. Frozen dataclass returned by `evaluate_pull_request(...)` in `personas/tpm/persona.py`. Only checks NOT in `_ADVISORY_CHECKS` contribute to the `conclusion` field. Composes into a `CheckRunResult` for GitHub. |
...
| Term | Definition |
|---|---|
| **GitHub Ruleset** | A GitHub Repository Ruleset that requires specific status checks to pass before merging. Grug creates rulesets to enforce **Hunt Plan** (`Grug - Chief`) on the default branch. Managed via the Repository Rulesets API. See [`services/_shared/github_rulesets_client.py`](services/_shared/github_rulesets_client.py). |
...
| **`grug.enforcement.state`** | DogStatsD gauge emitted by `emit_enforcement_metric()` (`observability.py`). Values: 1.0 `grug_managed`, 1.0 `opted_out`, 0.5 `external`, 0.0 `none`, -1.0 `error`. `error` means detection FAILED (auth/rate-limit), so enforcement is UNKNOWN rather than absent - it is deliberately distinguishable from `none` (#518). EVERY visible repo reports every poll cycle, including opted-out ones: leaving the gauge is never how a repo stops alerting, because Datadog holds a silent multi-alert group in its last state for 24h (ADR-0022). |
| **`opted_out`** | Gauge state (not an `EnforcementState`) for a repo Grug deliberately does not gate - either `tpm_enabled=false` or `force_disable_enforcement`. Reports 1.0, so it is healthy rather than a gap. |
| **Provably gated** | What the enforcement-gap monitor asserts: `grug.enforcement.state >= 0.5`. Its inverse covers both "no enforcement found" and "could not tell", which is why the monitor is not named after `none` alone. |
